...
monitoring, and infrastructure troubleshooting.
...
- **Services** — Pi-hole (ad-blocking DNS), Proxmox Backup Server, and a full
  InfluxDB + Telegraf + Grafana monitoring stack
...
| [monitoring.md](monitoring.md) | InfluxDB + Telegraf + Grafana stack and custom integrations |
...
- **Linux systems** — Proxmox, LXC containers, networking stacks, service accounts
- **Virtualization** — Proxmox clustering, VM/container management, backups
- **Monitoring & observability** — InfluxDB/Telegraf/Grafana pipeline, custom
  metrics integration for a service with no native agent plugin, Flux dashboards
...
**Observability:** Full metrics pipeline — firewall, hypervisor, and DNS sinkhole
all reporting into a time-series database with Grafana dashboards, including a
custom-built integration for a service lacking a native monitoring agent.
...
- **Phase 4:** Automated backups + full monitoring stack (June 2026)
- **Phase 5:** Planned — NAS deployment, additional services, SBC cluster (k3s)
...
- REST API session authentication and a vendor-side auth bug
- Linux service-account permissions and PATH debugging
